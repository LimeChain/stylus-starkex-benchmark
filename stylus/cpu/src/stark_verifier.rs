...
impl StarkVerifier {
    ...
    // Lyubo: remove self from the function signature
    fn read_last_fri_layer(ctx: &[U256]) {
        ...
        let last_layer_ptr = ctx[channel_ptr];
        ...
        for i in last_layer_ptr..last_layer_end {
            bad_input |= ctx[i] > PRIME_MINUS_ONE;
        }
        ...
        ctx[new_digest_ptr] = ctx[digest_ptr] + 1;

        let mut input_data = Vec::with_capacity(fri_last_layer_deg_bound + 1 * 32);
        for i in new_digest_ptr..fri_last_layer_deg_bound + 1 {
            input_data.extend_from_slice(&ctx[i].to_be_bytes::<32>());
        }

        ctx[digest_ptr] = uint!(keccak(&input_data).into());
        ctx[channel_ptr + 2] = 0;
        ctx[channel_ptr] = last_layer_end;

        require!(bad_input == 0, b"Invalid field element.");
        ctx[MM_FRI_LAST_LAYER_PTR] = last_layer_ptr;
    }
